/**
 * Complément de capture Marchandise — ventes portail EN_ATTENTE + encaissement
 * (2026-09-21). Une vente EN_ATTENTE doit exister (créée via le portail).
 */
...
async function main() {
	...
	// Filtre statut (nouvelle option « En attente de validation »).
	...
	if (await statutTrigger.isVisible().catch(() => false)) {
		...
		await capture(page, "22-ventes-filtre-statut");
		await page.keyboard.press("Escape");
		await page.waitForTimeout(600);
	}
	await capture(page, "23-vente-attente");

	// Refus : ouvrir la modale (motif) puis Retour.
	...
	if (await refuserBtn.isVisible().catch(() => false)) {
		...
		await page.waitForTimeout(600);
		await dlg
			.locator("#motif-refus-vente")
			.fill("Produit en rupture de stock");
		await page.waitForTimeout(400);
		await capture(page, "24-refus-dialog");
		await dlg.getByRole("button", { name: "Retour" }).click();
		await page.waitForTimeout(800);
	}

	// Valider → EN_COURS (décrémente le stock côté serveur).
	...
	if (await validerBtn.isVisible().catch(() => false)) {
		...
		await capture(page, "25-vente-validee");
	}
	...
	if (await encaisserBtn.isVisible().catch(() => false)) {
		...
		await dlg.waitFor({ timeout: 10000 });
		await page.waitForTimeout(600);
		const moyen = dlg.locator('[id*="moyen"], button[role="combobox"]');
		await moyen.first().click();
		await page.waitForTimeout(800);
		await page.getByRole("option").first().click();
		await page.waitForTimeout(500);
		await capture(page, "26-encaisser-dialog");
		await dlg
			.getByRole("button", { name: /Encaisser|Valider|Confirmer/ })
			.last()
			.click();
		await page.waitForTimeout(2500);
		await capture(page, "27-vente-payee");
	} else {
	...
	console.log("✅ Captures Marchandise (portail) terminées.");
}
