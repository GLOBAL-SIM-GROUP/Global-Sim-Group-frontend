...
5. **Pas d'état global non justifié** — état serveur → TanStack Query
   (`core/query`), session → `core/auth`, URL → search params du router,
   formulaire → TanStack Form, le reste → `useState`.
6. **Pas de nouvelle dépendance sans justification** — épinglée et documentée
   si ajoutée.
7. **Ne pas contourner les abstractions auth/permissions** — guards via
   `core/auth` + `redirect()`, permissions via `core/permissions` (UX only ;
   le backend reste la frontière de sécurité).
8. **Pas de secret dans le code frontend** — les variables exposées au
